...
class PhotoLibraryPermission: NSObject {
    ...
    private var didSelectPhotos: ((PHChange) -> ())?
    ...
    func onDidSelectPhotos(completion: ((PHChange) -> ())?) -> Self {
    ...
    private static func presentAlertSettings(from viewController: UIViewController?, completion: ((_ granted: Bool) -> ())?) {
        let alertVC = UIAlertController(title: "Photo library permission", message: "Please allow access to photo library", preferredStyle: .alert)
...
extension PhotoLibraryPermission: PHPhotoLibraryChangeObserver {
    ...
    func photoLibraryDidChange(_ changeInstance: PHChange) {
        self.didSelectPhotos?(changeInstance)
    }
}
